...
int nd_pam_authenticate_user (char * uuid_str, /*struct pam_user_info*/SessionInfo * user_info, pam_handle_t * pamh)
{
	...
	char    svrConnSuSessKeyNo[ND_UUID_LENGTH] = {0,};
	char 	svrConnPreSuSessKeyNo[ND_UUID_LENGTH] = {0,};
	...
	int sam_opermode = is_sam_oper_mode(sDataHomeDir);
        ...
        nd_log (NDLOG_TRC, "pam_opermode = %d", pam_opermode);

	/*
        if (!is_pam_oper_mode(sDataHomeDir))
                return PAM_SUCCESS;
	*/
	...
	*/
	...
	//syslog (LOG_ERR, "Checking connection to the API server.- server ip:[%s], server port:[%d]", auth_server_ip, authsvr_port);
	...
	*/
	if (bisAlive_server != true)
	{
		...
		if (strcmp (authsvr_emergency_act, "1") == 0 )	{
			...
			*/
...
#endif 
			nd_log(NDLOG_INF, "Unable to connect to the API server. Info = %s:%d, emergency mode = %s", auth_server_ip, authsvr_port,authsvr_emergency_act);
			...
			//
			///
			nd_log (NDLOG_ERR, "Unable to connect to the API server. Info = %s:%d, emergency mode = %s", auth_server_ip, authsvr_port,authsvr_emergency_act);
			return PAM_AUTH_ERR;
		}
		else								{
			...
			//
			///
			nd_log(NDLOG_INF, "Unable to connect to the API server.");
			return PAM_SUCCESS;
		}	
	}
	...
	//
	///
	nd_log (NDLOG_TRC, "Checking connection to the API server. bisAlive_server = true");
/*
	int pam_pri_no, pam_action, pam_logging, sam_pri_no, sam_action, sam_logging;
    	char *agt_auth_no, *ndshell_agtAuthNo;
...
*/
	/*header*/
	//sprintf ((char *)header.sAgentId, agent_id);
	...
	//
	///
	nd_log (NDLOG_INF, "start checking pam policy.(%s/%s)",user_info->remote_host, user_info->current_user);
	if (check_pam_policy (getPamRuleFilePath( sDataHomeDir), user_info->remote_host, user_info->current_user, &pam_pri_no, &agt_auth_no, &pam_action, &pam_logging))
		...
		//
		///
		nd_log (NDLOG_TRC, "Policy check result: Complies with the policy.");
...
#endif //_SUPP_DATE_
	{
		...
		nd_log (NDLOG_TRC, "user's session is executed in ndshell. Checking the SAM policy. [account:%s]",user_info->current_user);
		...
		nd_log (NDLOG_TRC, "sampolicy ... agtAuthNo (%s)", agtAuthNo);
	}

	if (pam_opermode == 0 )
	{

		nd_log(NDLOG_TRC, "pam_opermode IS OFF ..");
	...
	for (int i = 0 ; i < 3 ; i ++ )
	{
		...
		{
			pam_error(pamh, "Authentication failed: Invalid input. Attempt count: %d/3",i);
	...
	//nd_log (NDLOG_ERR, "certTpCode>>>>>>>>>>>:%s", certTpCode);
	...
	*/	
	...
	if (retval != HI_AUTH_RET_SUCCEED || hi_hiwareauth_ret.ret != 200 )
	{
		...
		//snprintf (certTpCode , sizeof (certTpCode), "%s", hi_hiwareauth_ret.certTpCode);
		//snprintf (certStepSeqNo , sizeof (certStepSeqNo), "%s", hi_hiwareauth_ret.certStepSeqNo);
        ...
        snprintf ( certTpCode,          sizeof (certTpCode),            "%s", hi_hiwareauth_ret.certTpCode);
        snprintf ( certAppTpCode,       sizeof (certAppTpCode),         "0");
        snprintf ( certStepSeqNo,       sizeof (certStepSeqNo),         "%s", hi_hiwareauth_ret.certStepSeqNo);
	...
	{	
		...
		*/
		...
		if (retval != HI_AUTH_RET_SUCCEED)
		{
                	...
                	snprintf (svrConnRstTpCode , sizeof (svrConnRstTpCode),    "%s", PAM_AUTH_FAIL);
                	snprintf (pamCertDtlAuthCode, sizeof (pamCertDtlAuthCode), "%s", PAM_CERT_DTL_AUTH_TWOFACT);
			snprintf (certSucesFailYn,    sizeof (certSucesFailYn),    "0");

			nd_log (NDLOG_TRC , "requestTwoFactAuthToApiserver IS FAIL hi_twofact_ret.certTpCode : %s",hi_twofact_ret.certTpCode);
			nd_log (NDLOG_TRC , "requestTwoFactAuthToApiserver IS FAIL hi_twofact_ret.certStepSeqNo : %s",hi_twofact_ret.certStepSeqNo);
			nd_log (NDLOG_TRC , "requestTwoFactAuthToApiserver IS FAIL g_sDataHiwareUserNumber : %s",g_sDataHiwareUserNumber);
...
*/
	...
	//nd_log (NDLOG_TRC, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  uuid_str: %s", uuid_str);
	memset (sDataEnv_var, 0x00 , sizeof (sDataEnv_var));

	snprintf (sDataEnv_var, sizeof (sDataEnv_var), HIWARE_SESSION_KEY_FORMAT, uuid_str);
	pam_putenv(pamh, sDataEnv_var);

	// nd_log (NDLOG_TRC, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  uuid_str SET : %s", sDataEnv_var);

nd_pam_authenticate_user:
	//nd_log(NDLOG_ERR, "pam_logging (%d)(%s)", pam_logging, userNo);
	if (pam_logging == LOGGING_ON) 	{
		...
		*/
		...
		memset (sDataEnv_var, 0x00 , sizeof (sDataEnv_var));
		snprintf (sDataEnv_var, sizeof (sDataEnv_var), HIWARE_LAST_AUTH_CODE_FORMAT, pamCertDtlAuthCode  );
		pam_putenv(pamh, sDataEnv_var);

		if (strcmp (pamCertDtlCode, PAM_SU_LOGIN) == 0 || strcmp (pamCertDtlCode, PAM_SU_LOGOUT) == 0)
			snprintf (pamCertDtlAuthCode, sizeof (pamCertDtlAuthCode), PAM_CERT_DTL_AUTH_SU_RULE);

		//nd_log (NDLOG_ERR, "certTpCode:%s", certTpCode);
		//nd_log(NDLOG_ERR, "write log start...>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>nd_pam_authenticate_user");
		logitem =  create_archive_log (svrConnStartTime, svrConnEndTime, svrConnRstTpCode, svrConnFailRsnCode, agtNo, agtConnFormTpCode,
						agtAuthNo, portNo, userIp, securStepNo, svrConnSessKey, svrConnSuSessKeyNo, svrConnPreSuSessKeyNo, connAcctId, switchAcctId, pamAgtAuthNo, userNo, pamCertDtlCode,pamCertDtlAuthCode,
	 ...
	 nd_log(NDLOG_ERR, "write log fin...");
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags,
	...
	*/
	...
	nd_log (NDLOG_TRC, "pam_op_mode IS (%s)/(%s)", pam_op_mode, getPamRuleFilePath( sDataHomeDir));

	/*
	if (strcmp (pam_op_mode, "1") != 0)
		return PAM_SUCCESS;
	*/
	...
	*/
	...
	//
	///
	nd_log (NDLOG_INF, ">> get agent id : [%s]",agent_id);
	nd_log (NDLOG_INF, ">> get sshd listening port : [%d]", g_nDataSshPort);
	...
	*/
	...
	nd_log (NDLOG_TRC, "GET_PAM_ITEM : TTY (%s)", tty);
		...
		nd_log (NDLOG_INF, "sDataEnv_var is [%s]", uuid_str);
		...
		if (sessionkey == NULL)
			sessionkey = getenv(ENV_HIWARE_SESSIONKEY);
                ...
                snprintf (svrConnSessKey, sizeof (svrConnSessKey), "%s", sessionkey ? sessionkey : "");

		memset (uuid_str, 0x00 , sizeof (uuid_str));

		snprintf (uuid_str, sizeof (svrConnSessKey), "%s", sessionkey);
		...
		if (presessionkey != NULL)
		{
			...
			snprintf (svrConnPreSuSessKeyNo, sizeof (svrConnPreSuSessKeyNo), "%s", presessionkey ? presessionkey : ""); 
		}		
			

		//CREATE NEW SU SESSION KEY
		su_uuid_str = generate_uuid();
		snprintf (svrConnSuSessKeyNo , sizeof (svrConnSuSessKeyNo), "%s", su_uuid_str ? su_uuid_str : "");
		...
		nd_log (NDLOG_TRC, "SU SESSION KEY : %s", svrConnSessKey);
		nd_log (NDLOG_TRC, "SU NEW_ESSION_KEY : %s", svrConnSuSessKeyNo);
		...
		nd_log (NDLOG_TRC, "CHEK SAM POLICY...		");
...
#endif //_SUPP_DATE_
		{
			...
			nd_log (NDLOG_TRC, "CHEK SAM POLICY...001");
		...
		nd_log (NDLOG_TRC, "CHEK PAM POLICY....(%s)[%s/%s]",getPamRuleFilePath( sDataHomeDir),info->remote_host, info->current_user);
		...
		if ( check_pam_policy (getPamRuleFilePath( sDataHomeDir), info->remote_host, info->current_user, &pam_pri_no, &agt_auth_no, &pam_action, &pam_logging))
                {
			nd_log (NDLOG_TRC, "CHEK PAM POLICY....1");
                        if (pam_action == PAM_ACT_RULE_DENY)
                        {
                                snprintf(svrConnFailRsnCode, sizeof(svrConnFailRsnCode), PAM_SVR_FAIL_OS_AUTH_FAIL);
                                snprintf(svrConnRstTpCode, sizeof(svrConnRstTpCode), "%s", PAM_AUTH_FAIL);
		...
		nd_log (NDLOG_TRC, "CHEK PAM POLICY....2");
		...
		nd_log (NDLOG_TRC, "SU LOGIN SUCCESS!!!!!!!");
		...
		if (service != NULL && (strcmp (service, STR_SU) == 0 || strcmp (service, STR_SUL) == 0))
		{
			...
			if (isNdShell == true )
			{
												...
												&sam_action, &sam_logging) == 1)
				{
					...
					snprintf (pamAgtAuthNo ,        sizeof (pamAgtAuthNo),          "%s", agt_auth_no ? agt_auth_no : "" );
					//snprintf (userNo ,              sizeof (userNo) ,               "%s", agent_id : agent_id : "" );
					snprintf (pamCertDtlCode ,      sizeof (pamCertDtlCode),        "%s", PAM_SU_LOGIN );
					snprintf (pamCertDtlAuthCode ,  sizeof (pamCertDtlAuthCode),    "%s", PAM_CERT_DTL_AUTH_SAM_RULE );
