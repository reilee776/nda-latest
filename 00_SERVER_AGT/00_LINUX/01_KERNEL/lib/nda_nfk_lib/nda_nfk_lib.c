...
/**
 * @brief 커널 드라이버에서 관리 중인 세션 수를 가져옵니다.
 *
 * 이 함수는 사용자 영역에서 커널 드라이버와 통신하여,
 * 현재 드라이버가 관리하고 있는 세션 개수를 조회합니다.
 *
 * - 드라이버와의 통신은 문자 디바이스 `/dev/nd_nix_chardev`를 통해 이루어집니다.
 * - IOCTL 명령어 `IOCTL_GET_CONNECTSESSIONCNT`를 사용하여 세션 수를 요청합니다.
 * - 이 명령어는 드라이버 내부 상태에 접근하여 현재 연결된 세션 수를 사용자 공간으로 전달합니다.
 *
 * @param cnt 세션 수가 저장될 버퍼의 포인터
 *            드라이버는 해당 포인터를 통해 세션 개수를 문자열 형태로 전달합니다.
 *
 * @return 성공 시 0, 실패 시 -1
 *
 * ⚠️ 이 인터페이스는 시스템 내부 전용입니다. 외부 문서화 금지.
 */
int sdk_get_NdaNfkDrv_ManagedSessionCnt(char * cnt)
{
    ...
    // IOCTL 호출로 현재 드라이버가 관리 중인 세션 수 요청
    ret = ioctl (fd, IOCTL_GET_CONNECTSESSIONCNT, cnt);
    if (ret < 0 )
    {
        // ioctl 실패 시 디바이스 닫고 -1 반환
        close (fd);
        return -1;
    }
...
int sdk_get_NdaNfkDrv_version(char * version)
{
    ...
    // IOCTL 호출: 드라이버로부터 버전 문자열 요청
	ret = ioctl (fd, IOCTL_GET_VERSION, version);
	if (ret < 0 )
	{
        // ioctl 실패 시 리소스 해제 후 종료
		close (fd);
		return -1;
	}
...
int sdk_NdaNfkDrv_start(void)
{
    ...
    // IOCTL 호출: 드라이버의 동작 모드 활성화 요청
    ret = ioctl (fd, IOCTL_ON_MODE, NULL);
    if (ret < 0 )
    {
           // ioctl 실패 시 디바이스 닫고 에러 반환
        close(fd);
        return -1;
    }
...
int sdk_get_NdaNfkDrv_state(char * sStatus)
{
    ...
    // IOCTL 호출: 드라이버 상태 조회 ("on" 또는 "off" 문자열 반환)
...
int sdk_add_NdaNfkDrv_service_policy(const struct cmd_service_rule_pars_data * service)
{
    ...
    // 전달된 서비스 정책 포인터가 NULL인 경우, 처리 중단
	if (service == NULL )
		return -1;
    ...
    // IOCTL 호출로 서비스 정책 추가 요청
    // 커널은 전달받은 service 구조체를 참조하여 정책을 등록함
	ret = ioctl (fd, IOCTL_ADD_SERVICE_POLICY, service);
	if (ret < 0 )
	{
        // IOCTL 실패 시 디바이스 닫고 오류 반환
		close(fd);
		return -1;
	}
...
int sdk_get_NdaNfkDrv_action_policy_index(const struct cmd_service_sub_rule_pars_data * action )
{
    ...
    // IOCTL 호출: 조건에 부합하는 액션 정책의 인덱스 요청
    ...
    // 커널에서 반환한 인덱스 값을 구조체 내 ret 필드에서 추출
    ret = action->ret;
